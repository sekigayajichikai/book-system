...
> 作成日: 2026-06-03 / 最終更新: 2026-06-03
...
| `/api/sync-drive` | GET | （外部: Google Drive API） | DriveファイルのファイルIDと最終更新日時を取得。「更新日: 2026年5月29日」の表示に使用 |
...
|--------------|---------|-------------------|
| OrgPicker | booking_org_groups, booking_organizations | 予約作成/編集で団体を選択する時（グループ→団体の2段階ドロップダウン） |
...
| コンポーネント | テーブル | どんな時に呼ばれるか |
|--------------|---------|-------------------|
| DetailPopover | calendar_events | イベントのタイトル・時間・説明・display_title・主な予定フラグを変更 |
